...
CREATE TABLE IF NOT EXISTS "customer_waivers" (
    ...
    "course_id" INTEGER COMMENT 'If course-specific',
    "trip_id" INTEGER COMMENT 'If trip-specific',
    "rental_id" INTEGER COMMENT 'If equipment rental',

    -- Waiver Content
    "waiver_template_id" INTEGER COMMENT 'Template used',
    ...
    -- Compliance
    "reviewed_by" INTEGER COMMENT 'Staff who reviewed',
...
-- COMMENTED OUT: waiver_templates table already created by migration 024_create_waivers_system.sql
-- Migration 024 has different schema: (id, name, type, title, content, legal_text, ...)
-- This migration 052 tried to create with: (id, waiver_type, template_name, padi_form_number, ...)
...
-- COMMENTED OUT: Cannot INSERT with these column names - table has different schema from migration 024
-- If PADI-specific waivers needed, add them using migration 024's schema or create ALTER TABLE migration
...
CREATE TABLE IF NOT EXISTS "waiver_reminders" (
    "id" SERIAL PRIMARY KEY,
    "customer_waiver_id" INTEGER NOT NULL,
    "customer_id" INTEGER NOT NULL,
