...
sudo chcon -t httpd_sys_rw_content_t <?php echo ROOT_DIR; ?></pre>
            ...
            <?php elseif ($step == 4): ?>
                ...
                <?php if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['load_demo'])): ?>
                    <?php
                    // Load demo data
                    try {
                        ...
                        // Get tenant ID
                        $tenant = $pdo->query("SELECT id FROM tenants ORDER BY id ASC LIMIT 1")->fetch();
                        $tenantId = $tenant['id'];

                        echo "<div class='alert alert-info'><strong>Loading Demo Data...</strong></div>";
                        echo "<pre class='console'>";

                        // Demo Customers
                        echo "Creating demo customers...\n";
                        $demoCustomers = [
                            ['John', 'Doe', 'john.doe@example.com', '555-0101', 'Open Water'],
                            ['Jane', 'Smith', 'jane.smith@example.com', '555-0102', 'Advanced Open Water'],
                            ['Mike', 'Johnson', 'mike.j@example.com', '555-0103', 'Rescue Diver'],
                            ['Sarah', 'Williams', 'sarah.w@example.com', '555-0104', 'Divemaster'],
                            ['Tom', 'Brown', 'tom.brown@example.com', '555-0105', 'Open Water'],
                            ['Lisa', 'Davis', 'lisa.d@example.com', '555-0106', 'Advanced Open Water'],
                            ['David', 'Wilson', 'david.w@example.com', '555-0107', 'Open Water'],
                            ['Emily', 'Taylor', 'emily.t@example.com', '555-0108', 'Rescue Diver']
                        ];

                        foreach ($demoCustomers as $customer) {
                            $pdo->prepare("INSERT INTO customers (tenant_id, first_name, last_name, email, phone, certification_level, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())")
                                ->execute([$tenantId, $customer[0], $customer[1], $customer[2], $customer[3], $customer[4]]);
                            echo "  ✓ Added customer: {$customer[0]} {$customer[1]}\n";
                        }
                        ...
                        $categories = [
                            ...
                            ['Wetsuits', 'Wetsuits and drysuits'],
                            ['Fins & Masks', 'Fins, masks, and snorkels'],
                            ['Dive Computers', 'Dive computers and gauges'],
                            ['Accessories', 'Dive accessories and gear']
                        ];

                        $categoryIds = [];
                        foreach ($categories as $cat) {
                            $pdo->prepare("INSERT INTO product_categories (tenant_id, name, description, created_at) VALUES (?, ?, ?, NOW())")
                                ->execute([$tenantId, $cat[0], $cat[1]]);
                        ...
                        $products = [
                            ['Scubapro MK25 EVO Regulator', $categoryIds['Regulators'], 599.99, 5, 2],
                            ['Atomic Z2 Regulator', $categoryIds['Regulators'], 549.99, 3, 2],
                            ['Cressi MC9 Compact Regulator', $categoryIds['Regulators'], 299.99, 8, 3],
                            ['Scubapro Hydros Pro BCD', $categoryIds['BCDs'], 899.99, 4, 2],
                            ['Zeagle Ranger BCD', $categoryIds['BCDs'], 649.99, 6, 2],
                            ['Cressi Aquaride BCD', $categoryIds['BCDs'], 399.99, 10, 3],
                            ['Scubapro Definition 5mm Wetsuit', $categoryIds['Wetsuits'], 449.99, 12, 3],
                            ['Cressi Endurance 3mm Wetsuit', $categoryIds['Wetsuits'], 249.99, 15, 4],
                            ['Mares Flexa 5mm Wetsuit', $categoryIds['Wetsuits'], 329.99, 8, 3],
                            ['Scubapro Jet Fins', $categoryIds['Fins & Masks'], 129.99, 20, 5],
                            ['Mares Avanti Quattro+ Fins', $categoryIds['Fins & Masks'], 99.99, 25, 6],
                            ['Cressi Big Eyes Evolution Mask', $categoryIds['Fins & Masks'], 59.99, 30, 8],
                            ['Atomic Frameless Mask', $categoryIds['Fins & Masks'], 89.99, 15, 5],
                            ['Suunto D5 Dive Computer', $categoryIds['Dive Computers'], 699.99, 5, 2],
                            ['Shearwater Peregrine', $categoryIds['Dive Computers'], 499.99, 7, 2],
                            ['Cressi Leonardo Dive Computer', $categoryIds['Dive Computers'], 299.99, 10, 3],
                            ['Dive Light 1000 Lumens', $categoryIds['Accessories'], 149.99, 12, 4],
                            ['Surface Marker Buoy', $categoryIds['Accessories'], 39.99, 25, 8],
                            ['Mesh Gear Bag', $categoryIds['Accessories'], 49.99, 30, 10],
                            ['Dive Knife Stainless', $categoryIds['Accessories'], 69.99, 15, 5]
                        ];

                        foreach ($products as $product) {
                            $sku = 'DEMO-' . str_pad(rand(1000, 9999), 4, '0', STR_PAD_LEFT);
                            $pdo->prepare("INSERT INTO products (tenant_id, category_id, name, sku, description, price, cost, stock_quantity, low_stock_threshold, is_active, created_at)
                                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, NOW())")
                                ->execute([
                                    $tenantId,
                                    $product[1],
                                    $product[0],
                                    $sku,
                                    'Demo product for ' . $product[0],
                                    $product[2],
                                    $product[2] * 0.6, // Cost is 60% of price
                                    $product[3],
                                    $product[4]
                                ]);
                            echo "  ✓ Added product: {$product[0]} (Stock: {$product[3]})\n";
                        }

                        // Demo Courses
                        echo "\nCreating demo courses...\n";
                        $courses = [
                            ['Open Water Diver', 'PADI Open Water certification course', 399.99, 3],
                            ['Advanced Open Water', 'PADI Advanced Open Water course', 349.99, 2],
                            ['Rescue Diver', 'PADI Rescue Diver certification', 449.99, 3],
                            ['Divemaster', 'PADI Divemaster professional course', 899.99, 8],
                            ['Enriched Air (Nitrox)', 'Nitrox specialty certification', 199.99, 1]
                        ];

                        foreach ($courses as $course) {
                            $pdo->prepare("INSERT INTO courses (tenant_id, name, description, price, duration_days, max_students, is_active, created_at)
                                          VALUES (?, ?, ?, ?, ?, 8, 1, NOW())")
                                ->execute([$tenantId, $course[0], $course[1], $course[2], $course[3]]);
                            echo "  ✓ Added course: {$course[0]}\n";
                        }

                        echo "\n========================================\n";
                        echo "Demo Data Summary:\n";
                        echo "  ✓ " . count($demoCustomers) . " customers\n";
                        ...
                        echo "<div class='alert alert-success'>";
                        echo "<strong>✓ Demo Data Loaded Successfully!</strong><br>";
                        echo "You can now explore Nautilus with sample data.";
                        echo "</div>";

                    } catch (Exception $e) {
                        echo "<div class='alert alert-danger'>";
                        echo "<strong>✗ Error Loading Demo Data</strong><br>";
                        echo htmlspecialchars($e->getMessage());
                        echo "</div>";
                    }
                    ?>
                <?php else: ?>
                    ...
                    <div class="card mb-3">
                        <div class="card-body">
                            <h5 class="card-title">🎯 Load Demo Data (Optional)</h5>
                            <p class="card-text">Load sample data to explore Nautilus features:</p>
                            <ul class="mb-3">
                                <li>8 demo customers with various certification levels</li>
                                <li>6 product categories (Regulators, BCDs, Wetsuits, etc.)</li>
                                <li>20 dive products with realistic pricing</li>
                                <li>5 training courses (Open Water, Advanced, etc.)</li>
                            </ul>
                            <form method="POST" class="d-inline">
                                <input type="hidden" name="load_demo" value="1">
                                <button type="submit" class="btn btn-info w-100" onclick="return confirm('Load demo data? This will add sample customers, products, and courses to your database.')">
                                    📦 Load Demo Data
                                </button>
                            </form>
                            <small class="text-muted d-block mt-2">Note: You can delete demo data later from the admin panel.</small>
                        </div>
                    </div>
                <?php endif; ?>
